...
class TestStaticQuantizedModule(QuantizationTestCase):
    ...
    def _test_conv_api_impl(
        ...
        # MKLDNN only supports symmetric quantization of weight
        if torch.backends.quantized.engine == 'mkldnn' and not all(zp == 0 for zp in W_zero_point):
            return
...
class TestDynamicQuantizedModule(QuantizationTestCase):
    ...
    def test_linear_api(self, batch_size, in_features, out_features, use_bias, use_default_observer):
        ...
        # MKLDNN only supports symmetric quantization of weight
        if torch.backends.quantized.engine == 'mkldnn' and W_zp != 0:
            return
    ...
    def test_lstm_api(self, dtype, bidirectional):
        ...
        if not (dtype == torch.float16 and torch.backends.quantized.engine in ("qnnpack", "mkldnn")):
    ...
    def test_gru_api(self):
        ...
        for dtype in [torch.qint8, torch.float16]:
            if dtype == torch.float16 and torch.backends.quantized.engine in ("qnnpack", "mkldnn"):
                # fp16 dynamic quant is not supported for qnnpack or mkldnn
                continue
