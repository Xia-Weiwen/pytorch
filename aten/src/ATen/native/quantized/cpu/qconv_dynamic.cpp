...
template <int kSpatialDim>
at::Tensor PackedConvWeightsMkldnn<kSpatialDim>::apply_dynamic(
    const at::Tensor& input,
    bool reduce_range) {
...
template at::Tensor PackedConvWeightsMkldnn<2>::apply_dynamic(
    const at::Tensor& input,
    bool reduce_range);

template at::Tensor PackedConvWeightsMkldnn<3>::apply_dynamic(
    const at::Tensor& input,
    bool reduce_range);
