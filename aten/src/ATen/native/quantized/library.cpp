...
TORCH_LIBRARY(quantized, m) {
  ...
  m.def(TORCH_SELECTIVE_SCHEMA("quantized::bmm(Tensor qa, Tensor qb, float scale, int zero_point)-> Tensor qc"));
  ...
  m.def(TORCH_SELECTIVE_SCHEMA("quantized::conv1d_gelu(Tensor qx, __torch__.torch.classes.quantized.Conv2dPackedParamsBase packed_weight, float output_scale, int output_zero_point) -> Tensor"));
